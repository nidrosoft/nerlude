...
const Sidebar = ({ project, activeTab, onTabChange }: Props) => {
    ...
    return (
        ...
        >
            <div
                className={`flex flex-col p-3 rounded-3xl bg-b-surface2 shadow-lg backdrop-blur-sm transition-all duration-300 ease-in-out ${
                    isExpanded ? "w-56" : "w-16"
                }`}
            >
                {/* Project Header */}
                <div className={`flex items-center mb-4 pb-4 border-b border-stroke-subtle ${isExpanded ? "px-2" : "justify-center"}`}>
                    <div className="flex items-center justify-center size-10 rounded-full border-[1.5px] border-primary1/15 bg-primary1/5 shrink-0">
                        <span className="text-lg">{project.icon}</span>
                    </div>
                    {isExpanded && (
                        <div className="ml-3 min-w-0 overflow-hidden">
                            <div className="font-medium text-t-primary truncate text-small">
                                {project.name}
                            </div>
                ...
                <nav className="flex flex-col gap-1">
                    {navItems.map((item) => (
                        ...
                        >
                            ...
                            {isExpanded && (
                                <span className="ml-3 text-small font-medium whitespace-nowrap">
                                    {item.label}
                                </span>
                            )}
                        </button>
                ...
                <Link
                    href="/dashboard"
                    className={`flex items-center rounded-xl text-t-secondary fill-t-secondary hover:bg-b-surface1 hover:text-t-primary hover:fill-t-primary transition-all duration-200 ${
                        isExpanded ? "px-3 py-2.5" : "p-2.5 justify-center"
                    }`}
